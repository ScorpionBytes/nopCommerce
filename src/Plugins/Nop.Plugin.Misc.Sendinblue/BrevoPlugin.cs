...
namespace Nop.Plugin.Misc.Brevo
{
    ...
    public class BrevoPlugin : BasePlugin, IMiscPlugin, IWidgetPlugin
    {
        ...
        {
            var result = new Version("4.70.3").CompareTo(new Version(targetVersion));
            if (result < 0)
            {
                await _localizationService.AddOrUpdateLocaleResourceAsync(new Dictionary<string, string>
                {
                ...
                var task = await _scheduleTaskService.GetTaskByTypeAsync("Nop.Plugin.Misc.Sendinblue.Services.SynchronizationTask");
                if (task != null)
                {
                    task.Name = BrevoDefaults.SynchronizationTaskName;
                    task.Type = BrevoDefaults.SynchronizationTask;
